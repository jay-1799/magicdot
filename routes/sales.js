    ...
    if (newInquiryList.length > 0) {
      for (let inquiry of newInquiryList) {
        if (inquiry.subject.split(" ").length > 5) {
          inquiry.subject =
            inquiry.subject.split(" ").slice(0, 5).join(" ") + "...";
        }
...
router
  ...
  .get(async (req, res) => {
    try {
      if (
        !req.session.user ||
        req.session.user.role !== "sales representative"
      ) {
        return res.status(200).redirect("/");
      }
      ...
      if (inquiryDetails.isProjectCreated) {
        const projectDetails = await projectData.getProjectById(
  ...
  .post(async (req, res) => {
    try {
      if (!req.session.user || req.session.user.role !== "sales representative")
        return res.status(200).redirect("/");
      req.params.inquiryId = validators.validateId(
        req.params.inquiryId,
        "Inquiry Id"
      );
      req.body.message = validators.validateString(req.body.message, "Message");
      const postMessage = await salesInquiryData.addNewMessage(
        req.params.inquiryId,
        req.session.user._id,
        req.body.message,
        req.files
      ...
      return res
        .status(200)
        .redirect("/sales/inquirydetails/" + req.params.inquiryId);
    } catch (e) {
...
router.route("/generateaccount/:id").get(async (req, res) => {
  try {
    ...
    var status = await sendMailData.generateCredentialsAndSendEmail(
      req.params.id,
      req.session.user._id
    );
    console.log(status);
...
router
  ...
  .post(async (req, res) => {
    try {
      ...
      var projectReq = await projectRequestData.createProjectRequest(
        req.params.inquiryId,
        req.body.totalCoverageArea,
        req.body.previousYearAnnualUsage,
        req.body.previousYearAnnualEnergyCost,
        req.body.address
      );
      var closeInquiry = await salesInquiryData.closeSalesInquiry(
        req.params.inquiryId
      );
