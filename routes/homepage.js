...
router.route("/").get(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .render("homepage", { title: "Magicdot - Home", isLoggedIn: isLoggedIn });
  } catch (e) {
...
router.route("/services").get(async (req, res) => {
  return res.status(200).render("services", { title: "Magicdot - Services" });
});

router.route("/projects").get(async (req, res) => {
  return res.status(200).render("project", { title: "Magicdot - Projects" });
});
