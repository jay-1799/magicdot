...
router.route("/").get(async (req, res) => {
  try {
    ...
    const inquiryDetails = await salesInquiryData.getInquiryById(
      customer.customerInquiry
    );
    const salesRep = await users.getUserById(
...
router.route("/customerAgreement").post(async (req, res) => {
  try {
    ...
    req.body.customerName = validators.validateName(
      req.body.customerName.replace(/\s+/g, ""),
      "first name"
    );
    const customer = await users.checkUserAgreement(
      req.body.email,
      req.body.datePicker
    );
...
router.route("/chat").get(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .render("chat", { title: "Chat", user: req.session.user });
  } catch (error) {
...
router.route("/payments/process-payment").post(async (req, res) => {
  try {
    ...
    paymentStatus = await customerPayment.newPayment(
      req.session.user._id,
      paymentPlan,
      new Date(),
...
router.route("/payments/:id").get(async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${paymentId}.pdf"`
    );
    doc.pipe(res);
