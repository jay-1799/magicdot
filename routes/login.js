  ...
  .post(async (req, res) => {
    try {
      ...
      if (userData.role === "sales representative")
        ...
        return res.status(200).redirect("/customer");
      else if (userData.role === "operational manager")
        return res.status(200).redirect("/operations");
      else if (userData.role === "onsite team")
        return res.status(200).redirect("/onsite");
    } catch (e) {
