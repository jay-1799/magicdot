...
const generateCredentialsAndSendEmail = async (inquiryId, salesRepId) => {
  try {
    ...
    const addInquiryInfoToCustomer =
      await usersData.addSalesInquiryIdToCustomer(newUser._id, inquiryId);
    const assignSalesRepToInquiry =
      await salesInquiryData.assignSalesRepToInquiry(
        inquiryData._id,
        salesRepId
      );
    const addCustomerIdToInquiry =
      await salesInquiryData.addCustomerIdToInquiry(
        inquiryData._id,
        newUser._id
      );
