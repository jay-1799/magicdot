...
const newPayment = async (customerId, paymentPlan, date, ammount) => {
  ...
  if (!insertPayment.acknowledged || !insertPayment.insertedId)
    throw { status: 500, message: "Could not create payment" };
...
const getPayments = async (customerId) => {
  customerId = customerId;
  const paymentDetailsCollection = await paymentDetails();
  const payments = await paymentDetailsCollection
...
const getTransaction = async (paymentId) => {
  paymentId = paymentId;
  const paymentDetailsCollection = await paymentDetails();
  const transaction = await paymentDetailsCollection.findOne({
